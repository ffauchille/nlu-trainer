...
class LivePane extends React.Component<LivePaneProps, LivePaneState> {
  ...
  renderChatMessage(chatMessage: ChatMessage, idx: number) {
    ...
    if (chatMessage.type === 'predict') {
      // user message
      elem = <div key={`chat-message-${idx}`} style={{
        width: "100%",
        textAlign:"left",
        paddingLeft: "5%"
      }}><Icon name="user" color="black"/><Label basic pointing="left">{chatMessage.text}</Label></div>
    } else if (chatMessage.type === 'prediction') {
      elem = <div key={`chat-message-${idx}`} style={{
        width: "100%",
        textAlign: "right",
        paddingRight: "5%"
      }}><Label basic pointing="right">{chatMessage.text + " "} <Label color="violet">{(chatMessage.score || -1).toFixed(2)}</Label></Label><Icon name="code branch" color="black"/></div>
    }
    return elem;
  }
  ...
  render() {
    return (
      <Grid style={{
        ...
        <Grid.Row style={{ backgroundColor: "#6435c9", color: "whitesmoke" }}><Grid.Column textAlign="center">{this.props.app.name}</Grid.Column></Grid.Row>
        <Grid.Row style={{ height: "350px", overflowY: "scroll", borderLeft: borderColor }}>
